package app.rive;

import android.graphics.Bitmap;
import android.view.TextureView;
import kotlin.Metadata;
import kotlin.jvm.functions.Function0;
import kotlin.jvm.internal.o;
import org.jetbrains.annotations.NotNull;

/* JADX INFO: loaded from: classes.dex */
@Metadata(d1 = {"\u0000\b\n\u0000\n\u0002\u0018\u0002\n\u0000\u0010\u0000\u001a\u00020\u0001H\n¢\u0006\u0002\b\u0002"}, d2 = {"<anonymous>", "Landroid/graphics/Bitmap;", "invoke"}, k = 3, mv = {1, 9, 0}, xi = 48)
final class RiveKt$Rive$8$1$1$1$onSurfaceTextureUpdated$1$1 extends o implements Function0<Bitmap> {
    final /* synthetic */ TextureView $this_apply;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    RiveKt$Rive$8$1$1$1$onSurfaceTextureUpdated$1$1(TextureView textureView) {
        super(0);
        this.$this_apply = textureView;
    }

    @Override // kotlin.jvm.functions.Function0
    @NotNull
    public final Bitmap invoke() {
        Bitmap bitmap = this.$this_apply.getBitmap();
        if (bitmap != null) {
            return bitmap;
        }
        throw new IllegalStateException("Bitmap no longer available; surface may have been destroyed");
    }
}
